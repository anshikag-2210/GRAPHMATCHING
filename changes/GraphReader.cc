    ...
    if (curtok_ == TOK_PARTITION_A) {
        ...
        while (curtok_ != TOK_SEMICOLON) {
            ...
            if (vmap.find(v) != vmap.end()) {
                throw ReaderException("Resident name already exists");
            }
            ...
            if (curtok_ != TOK_SEMICOLON) {
                match(TOK_COMMA);
                if (curtok_ == TOK_SEMICOLON) {
                    throw ReaderException(error_message("invalid data in file", TOK_COMMA, { TOK_SEMICOLON }));
                }
            }
        }
    }
        ...
        while (curtok_ != TOK_SEMICOLON) {
            ...
            if (curtok_ == TOK_LEFT_BRACE) {
                ...
                if (!is_number(lexer_->get_lexeme())) {
                    throw ReaderException("Expected only Numbers in quotas ");
                }
                ...
                if (curtok_ == TOK_COMMA) {
                    ...
                    upper_quota = to_integer(lexer_->get_lexeme());
                    if (upper_quota < lower_quota) {
                        throw ReaderException("lower quota should be less than upper quota");
                    }
                    match(TOK_STRING);
                }
            ...
            //check if vertex already exists
            if (vmap.find(v) != vmap.end()) {
                throw ReaderException("Hospital name already exists");
            }
            // add this vertex with the required quotas
            vmap.emplace(v, std::make_shared<Vertex>(v, lower_quota, upper_quota));
...
void GraphReader::read_preference_lists(BipartiteGraph::ContainerType& A, BipartiteGraph::ContainerType& B) {
    ...
    while (curtok_ != TOK_AT) {
        ...
        //check if vertex already exists
        if (A.find(a) == A.end()) {
            throw ReaderException("Invalid find Resident/ Hospital name");
        }
        // read and store the preference list
        PreferenceList& pref_list = A[a]->get_preference_list();
        if (pref_list.size() > 0) {
            throw ReaderException("Preference list for already exists");
        }
        ...
        while (curtok_ != TOK_SEMICOLON) {
            ...
            //check if vertex already exists
            if (B.find(b) == B.end()) {
                throw ReaderException("Cant find Resident/ Hospital name");
            }
            if (pref_list.find(B[b]) != pref_list.end()) {
                throw ReaderException("Resident/ Hospital name already included in preference list");
            }
